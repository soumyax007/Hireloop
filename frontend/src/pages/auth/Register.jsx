import { useState } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { Eye, EyeOff, ArrowRight, ArrowLeft, Check } from 'lucide-react';
import { useAuth } from '../../contexts/AuthContext';
import toast from 'react-hot-toast';

const SAU_LOGO = 'data:image/png;base64,';

const BRANCHES = [
  'Computer Science','Electronics & Communication','Mathematics & Computing',
  'Physics','Chemistry','Biotechnology','Economics','International Relations',
  'Legal Studies','Sociology','Literature','Environmental Science',
];

export default function Register() {
  const { register } = useAuth();
  const nav = useNavigate();
  const [step, setStep] = useState(1);
  const [showPw, setShowPw] = useState(false);
  const [loading, setLoading] = useState(false);
  const [validatingEmail, setValidatingEmail] = useState(false);
  const [error, setError] = useState('');
  const [form, setForm] = useState({
    email: '', password: '',
    firstName: '', lastName: '',
    branch: 'Computer Science', batch: 2025, cgpa: '',
  });

  const set = k => e => setForm(f => ({ ...f, [k]: e.target.value }));

  const validateEmail = email => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);

  const goToDetails = async () => {
    if (!form.email) { setError('Email is required'); return; }
    if (!validateEmail(form.email)) { setError('Please enter a valid email address'); return; }
    if (!form.password || form.password.length < 6) { setError('Password must be at least 6 characters'); return; }
    setError('');
    setValidatingEmail(true);
    try {
      const res = await fetch((import.meta.env.VITE_API_URL || '/api') + '/auth/validate-email', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: form.email })
      });
      const data = await res.json();
      if (!data.valid) { setError(data.error || 'Please use a real email address'); setValidatingEmail(false); return; }
    } catch { /* allow on network fail */ }
    finally { setValidatingEmail(false); }
    setStep(3);
  };

  const submit = async () => {
    if (!form.firstName) { setError('First name is required'); return; }
    setLoading(true); setError('');
    try {
      const d = await register({ ...form, role: 'student' });
      toast.success('Welcome to HireLoop! 🎉');
      nav('/student/dashboard');
    } catch (e) { setError(e.error || 'Registration failed. Please try again.'); }
    finally { setLoading(false); }
  };

  const pwStrength = pw => {
    if (!pw) return 0;
    let s = 0;
    if (pw.length >= 6) s++;
    if (pw.length >= 10) s++;
    if (/[A-Z]/.test(pw)) s++;
    if (/[0-9!@#$%^&*]/.test(pw)) s++;
    return s;
  };
  const strength = pwStrength(form.password);
  const strengthColor = strength <= 1 ? 'var(--red)' : strength <= 2 ? 'var(--yellow)' : 'var(--green)';
  const strengthLabel = ['', 'Weak', 'Fair', 'Good', 'Strong'][strength];

  return (
    <div style={{ minHeight: '100vh', display: 'flex', background: '#f5f5f7', fontFamily: 'var(--font)' }}>

      {/* ── Left Panel ── */}
      <div style={{
        width: 440, flexShrink: 0, background: 'linear-gradient(160deg, #0d1f3c 0%, #0a2952 50%, #103070 100%)',
        display: 'none', flexDirection: 'column', justifyContent: 'center',
        padding: '60px 52px', position: 'relative', overflow: 'hidden'
      }} className="auth-left">
        {/* Glow */}
        <div style={{ position: 'absolute', top: -100, right: -100, width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle, rgba(0,113,227,.15) 0%, transparent 70%)', pointerEvents: 'none' }} />

        <div style={{ position: 'relative', zIndex: 2 }}>
          {/* SAU Logo */}
          <img src={SAU_LOGO} alt="South Asian University" style={{ height: 72, width: 'auto', filter: 'brightness(0) invert(1) opacity(0.9)', marginBottom: 28 }} />

          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 44 }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(255,255,255,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none">
                <path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </div>
            <div>
              <div style={{ fontSize: 16, fontWeight: 700, color: '#fff' }}>HireLoop</div>
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,.4)', letterSpacing: '.04em' }}>SAU Placement Portal</div>
            </div>
          </div>

          <h1 style={{ fontFamily: 'var(--font-d)', fontSize: 36, fontWeight: 400, lineHeight: 1.15, color: '#fff', marginBottom: 14, letterSpacing: '-.5px' }}>
            Your career<br /><em style={{ fontStyle: 'italic', color: 'rgba(255,255,255,.4)' }}>starts here.</em>
          </h1>
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,.45)', lineHeight: 1.7, marginBottom: 40 }}>
            Join thousands of SAU students on India's smartest placement platform.
          </p>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {['AI-powered resume analysis', 'Live mock interviews with feedback', 'Direct recruiter connections', 'Real-time application tracking'].map(t => (
              <div key={t} style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 14 }}>
                <div style={{ width: 22, height: 22, borderRadius: '50%', background: 'rgba(48,209,88,.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <Check size={12} color="#30d158" strokeWidth={3} />
                </div>
                <span style={{ color: 'rgba(255,255,255,.7)' }}>{t}</span>
              </div>
            ))}
          </div>
        </div>
      </div>

      {/* ── Right Panel ── */}
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 'clamp(32px,5vw,56px) clamp(20px,4vw,40px)' }}>
        <div style={{ width: '100%', maxWidth: 440 }}>

          {/* Mobile header */}
          <div style={{ marginBottom: 32, display: 'flex', flexDirection: 'column', gap: 10 }} className="reg-mobile-header">
            <img src={SAU_LOGO} alt="South Asian University" style={{ height: 52, width: 'auto' }} />
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 28, height: 28, borderRadius: 7, background: '#1d1d1f', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
                  <path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                  <path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>
              <span style={{ fontWeight: 700, fontSize: 15, color: '#1d1d1f' }}>HireLoop</span>
            </div>
          </div>

          {/* Progress bar */}
          <div style={{ display: 'flex', gap: 5, marginBottom: 28 }}>
            {[1, 2, 3].map(s => (
              <div key={s} style={{ height: 3, flex: 1, borderRadius: 2, background: s <= step ? 'var(--accent)' : 'var(--border-light)', transition: 'background .3s' }} />
            ))}
          </div>

          {/* ── Step 1: Intro ── */}
          {step === 1 && (
            <div className="anim-in">
              <h2 style={{ fontSize: 24, fontWeight: 700, letterSpacing: '-.5px', marginBottom: 6, color: 'var(--text-1)' }}>Create student account</h2>
              <p style={{ fontSize: 14, color: 'var(--text-2)', marginBottom: 28 }}>Join South Asian University's official placement portal</p>

              <div style={{ background: 'linear-gradient(135deg, #e8f0fd, #f0e6ff)', border: '1px solid rgba(0,113,227,.15)', borderRadius: 14, padding: '18px 20px', marginBottom: 28, display: 'flex', gap: 14, alignItems: 'center' }}>
                <img src={SAU_LOGO} alt="SAU" style={{ height: 44, width: 'auto' }} />
                <div>
                  <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--text-1)', marginBottom: 2 }}>SAU Student Registration</div>
                  <div style={{ fontSize: 12, color: 'var(--text-2)', lineHeight: 1.5 }}>Access jobs, AI tools, and placement resources exclusive to SAU.</div>
                </div>
              </div>

              {[
                ['🎯', 'AI resume scorer with ATS feedback'],
                ['🎙️', 'Live mock interviews with real scores'],
                ['💼', 'Exclusive recruiter connections'],
              ].map(([icon, text]) => (
                <div key={text} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, fontSize: 14, color: 'var(--text-1)' }}>
                  <span style={{ fontSize: 18 }}>{icon}</span>
                  <span>{text}</span>
                </div>
              ))}

              <button className="btn btn-primary btn-full btn-lg" style={{ marginTop: 24 }} onClick={() => setStep(2)}>
                Get Started <ArrowRight size={16} />
              </button>

              <div style={{ textAlign: 'center', fontSize: 13, color: 'var(--text-2)', marginTop: 16 }}>
                Already have an account? <Link to="/login" style={{ color: 'var(--accent)', fontWeight: 500 }}>Sign in</Link>
              </div>
            </div>
          )}

          {/* ── Step 2: Credentials ── */}
          {step === 2 && (
            <div className="anim-in">
              <button onClick={() => setStep(1)} style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--text-2)', fontSize: 13, background: 'none', border: 'none', cursor: 'pointer', marginBottom: 20, fontFamily: 'var(--font)', padding: 0 }}>
                <ArrowLeft size={14} /> Back
              </button>
              <h2 style={{ fontSize: 22, fontWeight: 700, letterSpacing: '-.4px', marginBottom: 6, color: 'var(--text-1)' }}>Your credentials</h2>
              <p style={{ fontSize: 14, color: 'var(--text-2)', marginBottom: 24 }}>Use your SAU email for full access</p>

              {error && <div className="alert alert-error" style={{ marginBottom: 16 }}>{error}</div>}

              <div className="form-group">
                <label className="label">SAU Email address</label>
                <input className="input" type="email" placeholder="yourname@sau.int" value={form.email} onChange={set('email')} autoFocus />
                <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 5 }}>Use your official SAU email for institutional access</div>
              </div>

              <div className="form-group">
                <label className="label">Password</label>
                <div style={{ position: 'relative' }}>
                  <input className="input" type={showPw ? 'text' : 'password'} placeholder="Minimum 6 characters"
                    value={form.password} onChange={set('password')} style={{ paddingRight: 42 }} />
                  <button type="button" onClick={() => setShowPw(s => !s)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', border: 'none', background: 'none', color: 'var(--text-3)', cursor: 'pointer', padding: 0 }}>
                    {showPw ? <EyeOff size={15} /> : <Eye size={15} />}
                  </button>
                </div>
                {form.password && (
                  <div style={{ marginTop: 8 }}>
                    <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
                      {[1,2,3,4].map(i => (
                        <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: strength >= i ? strengthColor : 'var(--border-light)', transition: 'background .2s' }} />
                      ))}
                    </div>
                    <div style={{ fontSize: 11, color: strengthColor }}>{strengthLabel}</div>
                  </div>
                )}
              </div>

              <button className="btn btn-primary btn-full btn-lg" onClick={goToDetails} disabled={validatingEmail} style={{ marginTop: 8 }}>
                {validatingEmail
                  ? <><div className="spinner spinner-sm" style={{ borderTopColor: '#fff' }} /> Verifying…</>
                  : <>Continue <ArrowRight size={16} /></>}
              </button>
            </div>
          )}

          {/* ── Step 3: Student Details ── */}
          {step === 3 && (
            <div className="anim-in">
              <button onClick={() => setStep(2)} style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--text-2)', fontSize: 13, background: 'none', border: 'none', cursor: 'pointer', marginBottom: 20, fontFamily: 'var(--font)', padding: 0 }}>
                <ArrowLeft size={14} /> Back
              </button>
              <h2 style={{ fontSize: 22, fontWeight: 700, letterSpacing: '-.4px', marginBottom: 6, color: 'var(--text-1)' }}>Almost there!</h2>
              <p style={{ fontSize: 14, color: 'var(--text-2)', marginBottom: 24 }}>Tell us about your academic profile at SAU</p>
              {error && <div className="alert alert-error" style={{ marginBottom: 16 }}>{error}</div>}

              <div className="form-row">
                <div className="form-group">
                  <label className="label">First name</label>
                  <input className="input" placeholder="Arjun" value={form.firstName} onChange={set('firstName')} autoFocus />
                </div>
                <div className="form-group">
                  <label className="label">Last name</label>
                  <input className="input" placeholder="Mehta" value={form.lastName} onChange={set('lastName')} />
                </div>
              </div>

              <div className="form-row">
                <div className="form-group">
                  <label className="label">Branch / Programme</label>
                  <select className="input" value={form.branch} onChange={set('branch')}>
                    {BRANCHES.map(b => <option key={b}>{b}</option>)}
                  </select>
                </div>
                <div className="form-group">
                  <label className="label">Graduation Year</label>
                  <select className="input" value={form.batch} onChange={set('batch')}>
                    {[2024,2025,2026,2027,2028].map(y => <option key={y}>{y}</option>)}
                  </select>
                </div>
              </div>

              <div className="form-group">
                <label className="label">CGPA <span style={{ fontWeight: 400, color: 'var(--text-3)' }}>(optional)</span></label>
                <input className="input" type="number" step="0.01" min="0" max="10" placeholder="e.g. 8.5" value={form.cgpa} onChange={set('cgpa')} />
              </div>

              <button className="btn btn-primary btn-full btn-lg" onClick={submit} disabled={loading} style={{ marginTop: 8 }}>
                {loading
                  ? <><div className="spinner spinner-sm" style={{ borderTopColor: '#fff' }} /> Creating account…</>
                  : <>Create SAU Account <ArrowRight size={16} /></>}
              </button>

              <p style={{ fontSize: 12, color: 'var(--text-3)', textAlign: 'center', marginTop: 16, lineHeight: 1.6 }}>
                By creating an account, you agree to SAU HireLoop's Terms of Service and Privacy Policy.
              </p>
            </div>
          )}

          {/* SAU footer note */}
          <div style={{ marginTop: 24, display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'center' }}>
            <img src={SAU_LOGO} alt="SAU" style={{ height: 18, width: 'auto', opacity: 0.3 }} />
            <span style={{ fontSize: 11, color: 'var(--text-3)' }}>South Asian University · New Delhi, India</span>
          </div>
        </div>
      </div>

      <style>{`
        .auth-left { display: none !important; }
        .reg-mobile-header { display: flex; }
        @media (min-width: 768px) {
          .auth-left { display: flex !important; }
          .reg-mobile-header { display: none; }
        }
        @media (max-width: 480px) {
          .form-row { flex-direction: column; gap: 0; }
        }
        .anim-in { animation: fadeUp .35s var(--ease-out) both; }
        @keyframes fadeUp { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: none; } }
      `}</style>
    </div>
  );
}
