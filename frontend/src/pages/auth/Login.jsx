...
export default function Login() {
  ...
  const ROUTES = { student: '/student/dashboard', admin: '/admin/dashboard' };
    ...
    <div className="hl-auth-root">
      ...
      <div className="hl-auth-left">
        <div className={`hl-auth-left-inner${mounted ? ' hl-visible' : ''}`}>
          ...
          {/* SAU Logo */}
          <div style={{ marginBottom: 36, display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 16 }}>
            <img src={SAU_LOGO} alt="South Asian University" style={{ height: 72, width: 'auto', filter: 'brightness(0) invert(1) opacity(0.9)' }} />
            <div style={{ width: 1, height: 0 }} />
          </div>

          <div className="hl-auth-brand" style={{ marginBottom: 16 }}>
          ...
          <p style={{ fontSize: 12, color: 'rgba(255,255,255,.35)', marginBottom: 44, letterSpacing: '.02em' }}>South Asian University · Placement Portal</p>
      ...
      <div className="hl-auth-right">
        <div className={`hl-auth-card${mounted ? ' hl-visible' : ''}`}>
          ...
          {/* Mobile: SAU logo */}
          <div className="hl-auth-mobile-logo" style={{ flexDirection: 'column', gap: 10, marginBottom: 28 }}>
            <img src={SAU_LOGO} alt="South Asian University" style={{ height: 52, width: 'auto' }} />
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div className="hl-auth-logomark" style={{ width: 28, height: 28, borderRadius: 7, background: '#1d1d1f' }}>
          ...
          <h2 className="hl-auth-title">Sign in to HireLoop</h2>
          <p className="hl-auth-sub">South Asian University Placement Portal</p>
          ...
          <form onSubmit={submit} className="hl-auth-form">
            <div className="hl-form-group">
              <label className="hl-label">SAU Email address</label>
              <div className="hl-input-wrap">
                <Mail size={14} className="hl-input-icon" />
                <input className="hl-input" type="email" placeholder="yourname@sau.int"
          ...
          {/* SAU footer note */}
          <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid var(--border-light)', display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'center' }}>
            <img src={SAU_LOGO} alt="SAU" style={{ height: 20, width: 'auto', opacity: 0.4 }} />
            <span style={{ fontSize: 11, color: 'var(--text-3)' }}>South Asian University, New Delhi</span>
          </div>
        </div>
      </div>

      <style>{`
        .hl-auth-mobile-logo { display: flex; align-items: center; }
        .hl-role-tabs { display: flex; gap: 8px; margin-bottom: 20px; }
        .hl-role-tab {
          flex: 1; display: flex; align-items: center; justify-content: center; gap: 6px;
          padding: 9px 12px; border: 1.5px solid var(--border-light); border-radius: 10px;
        ...
        .hl-auth-title { font-size: 22px; font-weight: 700; letter-spacing: -.4px; color: var(--text-1); margin-bottom: 4px; }
        .hl-auth-sub { font-size: 13px; color: var(--text-2); margin-bottom: 22px; }
        .hl-auth-card {
          background: #fff; border-radius: 20px; padding: 36px 32px;
          width: 100%; max-width: 420px;
          box-shadow: 0 2px 40px rgba(0,0,0,.08), 0 1px 3px rgba(0,0,0,.04);
          opacity: 0; transform: translateY(12px);
          transition: opacity .5s var(--ease-out), transform .5s var(--ease-out);
        }
        .hl-auth-card.hl-visible { opacity: 1; transform: none; }
        @media (max-width: 767px) {
          .hl-auth-mobile-logo { display: flex !important; }
          .hl-auth-card { padding: 28px 22px; border-radius: 16px; }
        }
        @media (min-width: 768px) {
          .hl-auth-mobile-logo { display: none !important; }
        }
      `}</style>
    </div>
  );
