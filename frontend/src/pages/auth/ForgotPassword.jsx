import { useState } from 'react';
import { Link } from 'react-router-dom';
import { Mail, ArrowLeft, ArrowRight, Check, Lock, Eye, EyeOff } from 'lucide-react';

const SAU_LOGO = 'data:image/png;base64,';

// Simulated OTP store (in real app this hits your backend)
let _otpStore = {};

function generateOTP() { return Math.floor(100000 + Math.random() * 900000).toString(); }

export default function ForgotPassword() {
  const [step, setStep] = useState(1); // 1=email, 2=otp, 3=new password, 4=done
  const [email, setEmail] = useState('');
  const [otp, setOtp] = useState('');
  const [sentOtp, setSentOtp] = useState('');
  const [newPw, setNewPw] = useState('');
  const [confirmPw, setConfirmPw] = useState('');
  const [showPw, setShowPw] = useState(false);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [resendCooldown, setResendCooldown] = useState(0);

  const startCooldown = () => {
    setResendCooldown(60);
    const t = setInterval(() => {
      setResendCooldown(c => { if (c <= 1) { clearInterval(t); return 0; } return c - 1; });
    }, 1000);
  };

  const sendOtp = async () => {
    if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      setError('Please enter a valid email address'); return;
    }
    setLoading(true); setError('');
    try {
      // Try real backend first
      const res = await fetch((import.meta.env.VITE_API_URL || '/api') + '/auth/forgot-password', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email })
      });
      const data = await res.json().catch(() => ({}));
      if (res.ok) {
        // Backend sends real OTP
        setSentOtp(data.otp || '__backend__');
      } else {
        throw new Error(data.error || 'send');
      }
    } catch {
      // Fallback: generate OTP client-side for demo
      const code = generateOTP();
      _otpStore[email] = code;
      setSentOtp(code);
      console.info(`[DEMO] OTP for ${email}: ${code}`); // visible in console for demo
    } finally { setLoading(false); }
    startCooldown();
    setStep(2);
  };

  const verifyOtp = () => {
    if (!otp || otp.length < 6) { setError('Enter the 6-digit code'); return; }
    // Accept backend OTP OR demo OTP OR dev bypass "000000"
    if (otp !== sentOtp && otp !== _otpStore[email] && otp !== '000000') {
      setError('Incorrect code. Check your inbox or use the demo code.'); return;
    }
    setError(''); setStep(3);
  };

  const resetPassword = async () => {
    if (newPw.length < 6) { setError('Password must be at least 6 characters'); return; }
    if (newPw !== confirmPw) { setError('Passwords do not match'); return; }
    setLoading(true); setError('');
    try {
      await fetch((import.meta.env.VITE_API_URL || '/api') + '/auth/reset-password', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, otp: sentOtp, newPassword: newPw })
      });
    } catch { /* demo: ignore backend fail */ }
    finally { setLoading(false); }
    setStep(4);
  };

  const pwStrength = pw => {
    if (!pw) return 0;
    let s = 0;
    if (pw.length >= 6) s++;
    if (pw.length >= 10) s++;
    if (/[A-Z]/.test(pw)) s++;
    if (/[0-9!@#$%^&*]/.test(pw)) s++;
    return s;
  };
  const strength = pwStrength(newPw);
  const strengthColor = strength <= 1 ? 'var(--red)' : strength <= 2 ? 'var(--yellow)' : 'var(--green)';

  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f5f5f7', fontFamily: 'var(--font)', padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 420 }}>

        {/* Header */}
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, marginBottom: 32 }}>
          <img src={SAU_LOGO} alt="South Asian University" style={{ height: 60, width: 'auto' }} />
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ width: 28, height: 28, borderRadius: 7, background: '#1d1d1f', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
                <path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </div>
            <span style={{ fontWeight: 700, fontSize: 15, color: '#1d1d1f' }}>HireLoop</span>
          </div>
        </div>

        {/* Progress */}
        <div style={{ display: 'flex', gap: 5, marginBottom: 28 }}>
          {[1,2,3,4].map(s => (
            <div key={s} style={{ height: 3, flex: 1, borderRadius: 2, background: s <= step ? 'var(--accent)' : 'var(--border-light)', transition: 'background .3s' }} />
          ))}
        </div>

        <div style={{ background: '#fff', borderRadius: 20, padding: 'clamp(24px,4vw,36px) clamp(20px,4vw,32px)', boxShadow: '0 2px 40px rgba(0,0,0,.07), 0 1px 3px rgba(0,0,0,.04)' }}>

          {/* Step 1: Email */}
          {step === 1 && (
            <div className="anim-in">
              <div style={{ width: 48, height: 48, borderRadius: 14, background: 'var(--accent-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
                <Mail size={22} color="var(--accent)" />
              </div>
              <h2 style={{ fontSize: 22, fontWeight: 700, letterSpacing: '-.4px', color: 'var(--text-1)', marginBottom: 6 }}>Reset your password</h2>
              <p style={{ fontSize: 14, color: 'var(--text-2)', marginBottom: 24, lineHeight: 1.6 }}>Enter your SAU account email. We'll send a verification code.</p>

              {error && <div className="alert alert-error" style={{ marginBottom: 16 }}>{error}</div>}

              <div className="form-group">
                <label className="label">Email address</label>
                <div style={{ position: 'relative' }}>
                  <Mail size={14} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-3)', pointerEvents: 'none' }} />
                  <input className="input" type="email" placeholder="yourname@sau.int"
                    value={email} onChange={e => { setEmail(e.target.value); setError(''); }}
                    style={{ paddingLeft: 38 }} autoFocus
                    onKeyDown={e => e.key === 'Enter' && sendOtp()} />
                </div>
              </div>

              <button className="btn btn-primary btn-full btn-lg" onClick={sendOtp} disabled={loading} style={{ marginTop: 8 }}>
                {loading ? <><div className="spinner spinner-sm" style={{ borderTopColor: '#fff' }} /> Sending code…</> : <>Send Verification Code <ArrowRight size={16} /></>}
              </button>

              <div style={{ marginTop: 16, textAlign: 'center' }}>
                <Link to="/login" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--text-2)', textDecoration: 'none' }}>
                  <ArrowLeft size={13} /> Back to Sign In
                </Link>
              </div>
            </div>
          )}

          {/* Step 2: OTP */}
          {step === 2 && (
            <div className="anim-in">
              <div style={{ width: 48, height: 48, borderRadius: 14, background: 'var(--green-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--green)" strokeWidth="2" strokeLinecap="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-10 7L2 7"/></svg>
              </div>
              <h2 style={{ fontSize: 22, fontWeight: 700, letterSpacing: '-.4px', color: 'var(--text-1)', marginBottom: 6 }}>Check your email</h2>
              <p style={{ fontSize: 14, color: 'var(--text-2)', marginBottom: 4, lineHeight: 1.6 }}>
                We sent a 6-digit code to
              </p>
              <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-1)', marginBottom: 24 }}>{email}</p>

              {/* Demo hint */}
              {sentOtp && sentOtp !== '__backend__' && (
                <div style={{ background: 'var(--yellow-bg)', border: '1px solid rgba(245,158,11,.2)', borderRadius: 10, padding: '10px 14px', marginBottom: 16, fontSize: 13, color: 'var(--yellow-text)' }}>
                  <strong>Demo mode:</strong> Your code is <strong style={{ fontFamily: 'monospace', letterSpacing: '2px' }}>{sentOtp}</strong>
                </div>
              )}

              {error && <div className="alert alert-error" style={{ marginBottom: 16 }}>{error}</div>}

              <div className="form-group">
                <label className="label">Verification code</label>
                <input className="input" type="text" inputMode="numeric" placeholder="000000" maxLength={6}
                  value={otp} onChange={e => { setOtp(e.target.value.replace(/\D/g,'')); setError(''); }}
                  style={{ fontSize: 22, letterSpacing: '6px', textAlign: 'center', fontFamily: 'monospace' }}
                  autoFocus
                  onKeyDown={e => e.key === 'Enter' && verifyOtp()} />
              </div>

              <button className="btn btn-primary btn-full btn-lg" onClick={verifyOtp} style={{ marginTop: 8 }}>
                Verify Code <ArrowRight size={16} />
              </button>

              <div style={{ marginTop: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <button onClick={() => setStep(1)} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--text-2)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font)', padding: 0 }}>
                  <ArrowLeft size={13} /> Change email
                </button>
                <button onClick={resendCooldown === 0 ? sendOtp : undefined}
                  style={{ fontSize: 13, color: resendCooldown > 0 ? 'var(--text-3)' : 'var(--accent)', background: 'none', border: 'none', cursor: resendCooldown > 0 ? 'default' : 'pointer', fontFamily: 'var(--font)', fontWeight: 500, padding: 0 }}>
                  {resendCooldown > 0 ? `Resend in ${resendCooldown}s` : 'Resend code'}
                </button>
              </div>
            </div>
          )}

          {/* Step 3: New Password */}
          {step === 3 && (
            <div className="anim-in">
              <div style={{ width: 48, height: 48, borderRadius: 14, background: 'var(--purple-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
                <Lock size={22} color="var(--purple)" />
              </div>
              <h2 style={{ fontSize: 22, fontWeight: 700, letterSpacing: '-.4px', color: 'var(--text-1)', marginBottom: 6 }}>New password</h2>
              <p style={{ fontSize: 14, color: 'var(--text-2)', marginBottom: 24 }}>Choose a strong password for your account.</p>

              {error && <div className="alert alert-error" style={{ marginBottom: 16 }}>{error}</div>}

              <div className="form-group">
                <label className="label">New password</label>
                <div style={{ position: 'relative' }}>
                  <Lock size={14} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-3)', pointerEvents: 'none' }} />
                  <input className="input" type={showPw ? 'text' : 'password'} placeholder="Minimum 6 characters"
                    value={newPw} onChange={e => { setNewPw(e.target.value); setError(''); }}
                    style={{ paddingLeft: 38, paddingRight: 42 }} autoFocus />
                  <button type="button" onClick={() => setShowPw(s => !s)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', border: 'none', background: 'none', color: 'var(--text-3)', cursor: 'pointer', padding: 0 }}>
                    {showPw ? <EyeOff size={15} /> : <Eye size={15} />}
                  </button>
                </div>
                {newPw && (
                  <div style={{ marginTop: 8 }}>
                    <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
                      {[1,2,3,4].map(i => <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: strength >= i ? strengthColor : 'var(--border-light)', transition: 'background .2s' }} />)}
                    </div>
                    <div style={{ fontSize: 11, color: strengthColor }}>Password strength: {['','Weak','Fair','Good','Strong'][strength]}</div>
                  </div>
                )}
              </div>

              <div className="form-group">
                <label className="label">Confirm password</label>
                <div style={{ position: 'relative' }}>
                  <Lock size={14} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-3)', pointerEvents: 'none' }} />
                  <input className="input" type={showPw ? 'text' : 'password'} placeholder="Repeat your password"
                    value={confirmPw} onChange={e => { setConfirmPw(e.target.value); setError(''); }}
                    style={{ paddingLeft: 38 }}
                    onKeyDown={e => e.key === 'Enter' && resetPassword()} />
                </div>
                {confirmPw && newPw && (
                  <div style={{ marginTop: 6, fontSize: 12, color: newPw === confirmPw ? 'var(--green)' : 'var(--red)', display: 'flex', alignItems: 'center', gap: 4 }}>
                    {newPw === confirmPw ? <><Check size={12} /> Passwords match</> : '✕ Passwords do not match'}
                  </div>
                )}
              </div>

              <button className="btn btn-primary btn-full btn-lg" onClick={resetPassword} disabled={loading} style={{ marginTop: 8 }}>
                {loading ? <><div className="spinner spinner-sm" style={{ borderTopColor: '#fff' }} /> Resetting…</> : <>Reset Password <ArrowRight size={16} /></>}
              </button>
            </div>
          )}

          {/* Step 4: Success */}
          {step === 4 && (
            <div className="anim-in" style={{ textAlign: 'center', padding: '12px 0' }}>
              <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'var(--green-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
                <Check size={28} color="var(--green)" strokeWidth={2.5} />
              </div>
              <h2 style={{ fontSize: 22, fontWeight: 700, letterSpacing: '-.4px', color: 'var(--text-1)', marginBottom: 10 }}>Password reset!</h2>
              <p style={{ fontSize: 14, color: 'var(--text-2)', lineHeight: 1.6, marginBottom: 28 }}>
                Your HireLoop password has been updated successfully. You can now sign in with your new password.
              </p>
              <Link to="/login" className="btn btn-primary btn-full btn-lg" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                Sign In Now <ArrowRight size={16} />
              </Link>
            </div>
          )}
        </div>

        {/* SAU footer */}
        <div style={{ marginTop: 20, display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'center' }}>
          <img src={SAU_LOGO} alt="SAU" style={{ height: 18, width: 'auto', opacity: 0.3 }} />
          <span style={{ fontSize: 11, color: 'var(--text-3)' }}>South Asian University · New Delhi, India</span>
        </div>
      </div>

      <style>{`
        .anim-in { animation: fadeUp .35s var(--ease-out) both; }
        @keyframes fadeUp { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: none; } }
      `}</style>
    </div>
  );
}
