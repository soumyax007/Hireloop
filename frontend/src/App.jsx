...
import Competition from './pages/shared/Competition';

function RequireAuth({ allowedRoles }) {
  const { user, loading } = useAuth();
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12, fontFamily: 'var(--font)', color: 'var(--text-2)' }}>
  ...
  if (allowedRoles && !allowedRoles.includes(user.role)) {
    const def = { student: '/student/dashboard', recruiter: '/recruiter/dashboard', admin: '/admin/dashboard' };
    return <Navigate to={def[user.role] || '/login'} replace />;
  }
  return <Outlet />;
}
